...
public class Main {
     ...
     * 1) части кода, которые могут потоками выполняться параллельно;
     * 2) часть кода, которая должна потоками выполняться последовательно.
     * Для первого случая в методе не используется синхронизация, а для второго - используется synchronized с указываем
     * монитора.
     * Консоль:
     * Thread-0 Not syn: Start // <- потоки на старте выполняют код параллельно
     * .
