 ...
 * Например, если в каком-то потоке у вас проводятся изменения в БД и вы вызовете завершение этого потока в том месте,
 * где производится работа с данными - то эти данные могут быть утеряны.
 * Для остановки потока в Java следует использовать метод interrupt(). Метод interrupt() устанавливает флаг прерывания
 * для потока, что сигнализирует потоку о необходимости завершить свою работу.
 * Метод interrupt() поток сразу не остановит. У потока есть внутри флаг isInterrupted(), который переходит в состояние true,
 * когда мы вызываем метод interrupt().
 * InterruptedException - это исключение, которое выбрасывается в потоке, когда он прерывается другим потоком.
 * Пока поток спит, то он не воспринимает команды. И если в тот момент, пока поток спит будет вызван interrupt(), то поток
 * спит и не реагирует на него и перевести свой флаг isInterrupted() = true и поэтому возникает исключение InterruptedException.
 ...
 * 2) когда поток находится в блокирующем состоянии, например, при попытке приобрести блокировку или получить доступ
 * к синхронизированному ресурсу.
 ...
 * Здесь Поток может либо обработать исключение InterruptedException и продолжить свою работу, либо повторно выбросить исключение,
 * например throw new RuntimeException() чтобы его обработал вызывающий поток.
 *
 * Консоль:
 * methodOne() started
 * .
...
public class Main {
    ...
    /**
     * Метод, использующий устаревшую обработку прерывания (не рекомендуется использовать)
     */
    public static void methodTwo() {
        ...
        try {
            Thread.sleep(3000);
            threadTwo.stop(); // Помечен как устаревший, показывает ошибку компиляции, но запускается и вызывает UnsupportedOperationException
